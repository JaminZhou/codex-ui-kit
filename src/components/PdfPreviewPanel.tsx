...
export function PdfPreviewPanel({
  ...
  const ready = !disabled && status === "ready" && count > 0;
  ...
  return (
    <section {...props} aria-label={title} aria-busy={status === "loading" || undefined}
      ...
      data-disabled={disabled || undefined} data-status={status} data-page={page} data-page-count={count} role="region">
      <header className="codex-ui-pdf-preview__header">
        ...
        <div className="codex-ui-pdf-preview__paging">
          <button aria-label="Previous page" disabled={!ready || page <= 1} onClick={() => { if (ready) onPageChange(page - 1); }} type="button">{glyph("previous")}</button>
          <span aria-label={`Page ${page} of ${count}`} aria-live="polite">{page}/{count}</span>
          <button aria-label="Next page" disabled={!ready || page >= count} onClick={() => { if (ready) onPageChange(page + 1); }} type="button">{glyph("next")}</button>
        </div>
        <div className="codex-ui-pdf-preview__actions">
          {onAnnotatingChange ? <button aria-label={annotating ? "Annotating" : "Annotate"} aria-pressed={annotating}
            className="codex-ui-pdf-preview__annotate" disabled={!ready} onClick={() => { if (ready) onAnnotatingChange(!annotating); }} type="button">
          ...
          <Menu align="end" className="codex-ui-pdf-preview__zoom-menu" label="PDF zoom" style={{ width: 160 }}
            ...
            {[25, 50, 100, 150, 200].map((zoom) => <MenuItem disabled={!ready} key={zoom} onSelect={() => { if (ready) onZoomChange(zoom); }}>{zoom}%</MenuItem>)}
            <MenuItem disabled={!ready} onSelect={() => { if (ready) onZoomChange("fit"); }}>Zoom to fit</MenuItem>
          </Menu>
          {onDownload ? <button aria-label="Download" disabled={!ready} onClick={() => { if (ready) onDownload(); }} type="button">{glyph("download")}</button> : null}
          {onOpen ? <div className="codex-ui-pdf-preview__open-group">
            <button aria-label={openLabel} className="codex-ui-pdf-preview__open" disabled={disabled || status === "loading"} onClick={() => { if (!disabled && status !== "loading") onOpen(); }} type="button">{icons.open}<span>Open</span></button>
            {onOpenOptions ? <button aria-label="Open options" className="codex-ui-pdf-preview__open-options" disabled={disabled || status === "loading"} onClick={() => { if (!disabled && status !== "loading") onOpenOptions(); }} type="button">{icons.options ?? glyph("chevron")}</button> : null}
          </div> : null}
        </div>
      </header>
      <div {...viewportProps} className={["codex-ui-pdf-preview__viewport", viewportProps?.className].filter(Boolean).join(" ")} ref={viewportRef}>
        {status === "loading" ? <div className="codex-ui-pdf-preview__state" role="status">Loading PDF</div>
          : status === "error" ? <div className="codex-ui-pdf-preview__state" role="alert">{errorDescription}{onRetry ? <button disabled={disabled} onClick={() => { if (!disabled) onRetry(); }} type="button">Retry preview</button> : null}</div>
          : children}
      </div>
    </section>
  );
