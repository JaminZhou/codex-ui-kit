...
are never approved by this probe. Cleanup declines any unresolved request.

The observed run approved only `approval-proof.txt`, received completed
file-change and turn events, and verified exact file bytes and final reply.
...
same call completing with one receiver thread in `agentsStates`. The live UI
renders the Agent activity row at 1180px, captures its computed font and
geometry, and the real Stop button interrupts the owning turn. The probe then
requires `turn/completed.status === "interrupted"`, visible `Stopped` status,
and the settled screenshot. The disposable workspace remains write-disabled;
the child prompt forbids shell, network, and file writes, and the created
...
The `check:live-long-thread` probe now runs inside full playground acceptance as
a signed-in gate. It runs twelve short no-tool turns on one real thread, asserts
repeated live bindings keep one stable thread ID, then scrolls away and follows
the latest turn again at 1180px and 720px. It records the viewport scroll
metrics and no-overflow screenshots in a disposable read-only workspace. The
