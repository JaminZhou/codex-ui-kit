...
const frame = "workspace-composer-current-26-911-github-mentioned";
const directory = await mkdtemp(
  join(tmpdir(), "ui-kit-composer-resource-mention-26-911-"),
);
...
async function capture(width, suffix) {
  ...
  try {
    ...
    await writeFile(
      join(directory, `composer-resource-mention-26-911-${width}-${suffix}.png`),
      screenshot,
    );
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        ...
        0,
        `${width}px current 26.911 mention fixture drifted`,
      );
    } finally {
...
console.log(
  JSON.stringify({
    ...
    replayEvidence: "current 26.911 public GitHub resource mention",
    widths: [1180, 720],
  }),
);
