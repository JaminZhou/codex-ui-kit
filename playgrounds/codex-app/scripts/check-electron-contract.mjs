...
try {
  ...
  const sectionMenu = page.getByRole("menu", { name: "Section submenu" });
  ...
  if (
    ...
    throw new Error(
      `Electron project menu lifecycle failed: ${JSON.stringify({ projectMenuContract, projectMenuFocusReturned, projectMenuIcons, sectionFocusReturned, sectionMenuContract })}`,
    );
  }
  if (
    ...
    throw new Error(
      `Electron Section submenu contract failed: ${JSON.stringify(sectionMenuContract)}`,
    );
  }
...
for (const integrationScene of [
  ...
  },
  {
    currentSidebar: true,
    frame: "integration-plugins-current-26-825",
    id: "electron-current-integration-plugins-light",
    scenario: "workspace-workflow",
    theme: "light",
    view: "plugins",
  },
  {
    currentSidebar: true,
    frame: "integration-plugins-current-26-825",
    id: "electron-current-integration-plugins-light-compact",
    scenario: "workspace-workflow",
...
for (const scheduledDetailScene of [
  {
    ...
    id: "electron-current-scheduled-26-915-detail-wide",
    scenario: "workspace-workflow",
    theme: "dark",
    view: "automations",
  },
  {
    currentSidebar: true,
    frame: "scheduled-current-26-915-detail",
    id: "electron-current-scheduled-26-915-detail-compact",
    scenario: "workspace-workflow",
    sidebarState: "compact-collapsed",
    theme: "dark",
    view: "automations",
  ...
  },
  {
    ...
    id: "electron-current-scheduled-26-915-detail-error",
    scenario: "workspace-workflow",
    theme: "dark",
    view: "automations",
  },
  {
    currentSidebar: true,
    frame: "scheduled-current-26-915-detail-error",
    id: "electron-current-scheduled-26-915-detail-error-compact",
    scenario: "workspace-workflow",
    sidebarState: "compact-collapsed",
    theme: "dark",
    view: "automations",
...
try {
  ...
  if (
    ...
    measured.controlCount !== 3 ||
    measured.pressedControls.filter((pressed) => pressed === "true").length !== 1 ||
    !measured.heading ||
...
const currentSettingsConnections917Scene = {
  ...
  id: "settings-connections-current-26-917-electron-contract",
  scenario: "workspace-workflow",
  view: "workspace",
  windowSize: { height: 600, width: 800 },
};
...
try {
  ...
  const initial = await currentSettingsConnections917Page.evaluate(() => {
    ...
    const hostChromeInset = Number.parseFloat(
      getComputedStyle(appMain).paddingBlockStart,
    );
    const main = rect(
      document.querySelector(".codex-ui-settings-shell__main"),
    );
    const navigation = rect(
    ...
    return {
      ...
      normalizedShell: {
        main: {
          ...main,
          height: main?.height + hostChromeInset,
          top: main?.top - hostChromeInset,
        },
        navigation: {
          ...navigation,
          height: navigation?.height + hostChromeInset,
          top: navigation?.top - hostChromeInset,
        },
      },
  ...
  if (
    ...
    !closeEnough(initial.normalizedShell.main?.height, 600) ||
    !closeEnough(initial.normalizedShell.navigation?.left, 0) ||
    ...
    throw new Error(
      `Electron current 26.917 Settings Connections geometry failed: ${JSON.stringify(initial)}`,
    );
  }

  for (const [label, buttonName] of [
    ["Control other devices", "Set up"],
    ["SSH", "Add"],
    ["Control this Mac", "Refresh devices"],
  ]) {
    await currentSettingsConnections917Page
    ...
    if (
      ...
      throw new Error(`${label} did not expose its pressed state in Electron.`);
    }
    if (label !== "Control this Mac") {
      await currentSettingsConnections917Page
...
async function currentMarkdown26825MediaContract(page) {
  return page.evaluate(() => {
    const rect = (element) => {
      const value = element?.getBoundingClientRect();
      return value
    ...
    const response = (id) => {
      ...
      return {
        ...
        media: media
          ? {
              disabled:
                media instanceof HTMLButtonElement ? media.disabled : false,
              href: media.getAttribute("href"),
              label: media.getAttribute("aria-label"),
              state:
                media.getAttribute("data-markdown-image-state") ?? "ready",
              style: style(media),
              tag: media.tagName,
            }
          : null,
    ...
    return {
      ...
      loaded: response("assistant-markdown-current-26-825-media-loaded"),
...
for (const currentMarkdown26825MediaScene of [
  {
    currentSidebar: true,
    frame: "markdown-current-26-825-media-loaded",
    id: "electron-markdown-current-26-825-media-loaded",
    scenario: "markdown-current-26-825-media",
  },
  {
    currentSidebar: true,
    frame: "markdown-current-26-825-media-loaded",
    id: "electron-markdown-current-26-825-media-loaded-compact",
    scenario: "markdown-current-26-825-media",
    sidebarState: "hidden",
    windowSize: { height: 680, width: 720 },
  },
  {
    currentSidebar: true,
    frame: "markdown-current-26-825-media-loaded",
    id: "electron-markdown-current-26-825-media-preview",
    scenario: "markdown-current-26-825-media",
  },
  {
    currentSidebar: true,
    frame: "markdown-current-26-825-media-unavailable",
    id: "electron-markdown-current-26-825-media-unavailable",
    scenario: "markdown-current-26-825-media",
  },
  {
    currentSidebar: true,
    frame: "markdown-current-26-825-media-unavailable",
    id: "electron-markdown-current-26-825-media-unavailable-compact",
    scenario: "markdown-current-26-825-media",
    sidebarState: "hidden",
    windowSize: { height: 680, width: 720 },
  },
  ...
  const { app: markdownMediaApp, page: markdownMediaPage } =
    await launchScene(currentMarkdown26825MediaScene, { capture: false });
  try {
    const compact = currentMarkdown26825MediaScene.id.endsWith("-compact");
    const expectsUnavailable =
      currentMarkdown26825MediaScene.frame.endsWith("media-unavailable");
    ...
    const contract = await currentMarkdown26825MediaContract(markdownMediaPage);
    const loaded = contract.loaded;
    ...
    if (
      ...
      loaded.paragraphs.length !== 2 ||
      Math.abs(loaded.paragraphs[0].rect.width - expectedMediaWidth) > 0.1 ||
      Math.abs(loaded.paragraphs[0].rect.height - 74) > 0.1 ||
      loaded.paragraphs[1].marginBlockStart !== "14px" ||
      ...
      (expectsUnavailable &&
        (!unavailable?.root?.rect ||
          ...
          unavailable.paragraphs.length !== 2 ||
          Math.abs(unavailable.paragraphs[0].rect.width - expectedMediaWidth) >
            0.1 ||
          Math.abs(unavailable.paragraphs[0].rect.height - 120) > 0.1 ||
          unavailable.paragraphs[1].marginBlockStart !== "14px" ||
          ...
          !unavailable.icon?.rect ||
          Math.abs(unavailable.icon?.rect.width - 28) > 0.1 ||
          Math.abs(unavailable.icon?.rect.height - 28) > 0.1 ||
          unavailable.actions.length !== 4)) ||
      contract.horizontalOverflow !== 0
    ) {
      throw new Error(
        `${currentMarkdown26825MediaScene.id}: current 26.825 Markdown media Electron contract failed: ${JSON.stringify({ contract, nativeBounds })}`,
      );
    }

    if (currentMarkdown26825MediaScene.id.endsWith("-preview")) {
      ...
      const preview = await dialog.evaluate((element) => {
        ...
        return {
          ...
          caption: caption
            ...
            : null,
          image: rect(image),
          imageRadius: image ? getComputedStyle(image).borderRadius : null,
          rect: rect(element),
          toolbar: rect(
            element.querySelector(".codex-ui-image-preview__zoom-toolbar"),
          ),
        };
      });
      ...
      if (
        ...
        !preview.image ||
        !preview.toolbar ||
        !preview.caption?.rect ||
        preview.activeLabel !== "Close image preview" ||
        preview.bodyOverflow !== "hidden" ||
        preview.rect.width !== 1180 ||
        preview.rect.height !== 820 ||
        Math.abs(preview.image.left - 566) > 0.1 ||
        Math.abs(preview.image.top - 338.71875) > 0.1 ||
        preview.image.width !== 48 ||
        preview.image.height !== 48 ||
        preview.imageRadius !== "12.5px" ||
        preview.caption.text !== "OpenAI favicon" ||
        Math.abs(preview.caption.rect.left - 527.96875) > 0.2 ||
        Math.abs(preview.caption.rect.top - 697.4375) > 0.2 ||
        Math.abs(preview.caption.rect.width - 124.046875) > 0.2 ||
        Math.abs(preview.caption.rect.height - 34.5625) > 0.2 ||
        ...
        throw new Error(
          `${currentMarkdown26825MediaScene.id}: current 26.825 Markdown media preview Electron contract failed: ${JSON.stringify(preview)}`,
        );
      }
      ...
      if (!(await trigger.evaluate((element) => element === document.activeElement))) {
        throw new Error(
          `${currentMarkdown26825MediaScene.id}: Markdown preview did not restore focus.`,
        );
      }
    }
  } finally {
...
const currentMcp26825SuccessScene = {
  currentSidebar: true,
...
const {
  app: currentMcp26825SuccessApp,
  page: currentMcp26825SuccessPage,
} = await launchScene(currentMcp26825SuccessScene, { capture: false });

try {
  const timeline = currentMcp26825SuccessPage.getByRole("button", {
    name: "Worked for 20s",
  });
  if ((await timeline.getAttribute("aria-expanded")) !== "false") {
    throw new Error("26.825 Electron MCP success should start collapsed.");
  }
  await timeline.click();
  await currentMcp26825SuccessPage
    .getByRole("button", {
      name: "Used OpenAI Developer Docs integration",
    })
    .click();
  const state = await currentMcp26825SuccessPage.evaluate(() => {
    const answer = document.querySelector(
      '[data-item-id="assistant-current-mcp-26-825-success"] .demo-current-mcp-answer',
    );
    return {
      answer:
  ...
  if (
    ...
    state.href !== "https://learn.chatgpt.com/docs/extend/mcp"
  ) {
    throw new Error(
      `26.825 Electron MCP success drifted: ${JSON.stringify(state)}`,
    );
  }
} finally {
  await currentMcp26825SuccessApp.close();
}

const currentMcp26903Scene = {
  currentSidebar: true,
  frame: "mcp-current-26-903-success",
  id: "electron-current-mcp-26-903-success",
  scenario: "mcp-current-26-903-success",
  sidebarState: "hidden",
  summaryState: "hidden",
};
const {
  app: currentMcp26903App,
  page: currentMcp26903Page,
} = await launchScene(currentMcp26903Scene, { capture: false });

try {
  const timeline = currentMcp26903Page.getByRole("button", {
    name: "Worked for 48s",
  });
  if ((await timeline.getAttribute("aria-expanded")) !== "false") {
    throw new Error("26.903 Electron MCP success should start collapsed.");
  }
  await timeline.click();
  await currentMcp26903Page
    .getByRole("button", {
      name: "Used OpenAI Developer Docs integration",
    })
    .click();
  const state = await currentMcp26903Page.evaluate(() => {
    const answer = document.querySelector(
      '[data-item-id="assistant-current-mcp-26-903-success"] .demo-current-mcp-answer',
    );
    return {
      answer:
        answer instanceof HTMLElement
  ...
  const toggle = currentMcp26903Page.getByRole("button", {
    name: "Toggle summary",
  });
  await toggle.click();
  const summary = await currentMcp26903Page.evaluate(() => {
    const dock = document.querySelector(
      ".demo-current-mcp-source-summary-dock",
    );
    const panel = dock?.querySelector(
  ...
  await currentMcp26903App.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0]?.setContentSize(720, 680);
  });
  await currentMcp26903Page.waitForFunction(
    () => window.innerWidth === 720 && window.innerHeight === 680,
    undefined,
    { timeout: 5_000 },
  );
  const compactOverflow = await currentMcp26903Page.evaluate(
    () => document.documentElement.scrollWidth - document.documentElement.clientWidth,
  );
  if (
    state.title !== "Find official Codex MCP page" ||
    JSON.stringify(state.callLabels) !==
      JSON.stringify(["Search OpenAI docs · 3 calls", "Fetch OpenAI doc"]) ||
    state.errorCards !== 0 ||
    state.rowDisclosures !== 0 ||
    state.overflow > 1 ||
    state.answer !==
      "Model Context Protocol https://learn.chatgpt.com/docs/extend/mcp#supported-mcp-features" ||
    state.href !==
    ...
    !summary.labels.includes("Trailglass") ||
    !summary.labels.includes("AppKitInspector") ||
    !summary.labels.includes("openai-docs-mcp") ||
    compactOverflow > 1
  ) {
    throw new Error(
      `26.903 Electron MCP success drifted: ${JSON.stringify({ compactOverflow, state, summary })}`,
    );
  }
} finally {
...
const {
  app: currentMcp26825RecoveryApp,
  page: currentMcp26825RecoveryPage,
} = await launchScene(currentMcp26825RecoveryScene, { capture: false });

try {
  for (const duration of ["Worked for 20s", "Worked for 10s"]) {
    await currentMcp26825RecoveryPage
      .getByRole("button", { exact: true, name: duration })
      .click();
  }
  const groups = currentMcp26825RecoveryPage.getByRole("button", {
    exact: true,
    name: "Used OpenAI Developer Docs integration",
  });
  ...
  const readState = () =>
    currentMcp26825RecoveryPage.evaluate(() => {
      const rect = (element) => {
        const bounds = element?.getBoundingClientRect();
        return bounds
          ? {
      ...
      return {
        answers: Array.from(
          document.querySelectorAll(".demo-current-mcp-answer"),
          (element) =>
            element instanceof HTMLElement
              ? element.innerText.replace(/\s+/g, " ").trim()
              : null,
        ),
        callGroups: Array.from(
          document.querySelectorAll(".codex-ui-mcp-tool-call-group"),
          (group) =>
            Array.from(
              group.querySelectorAll(".codex-ui-tool-call__label"),
              (element) => element.textContent?.trim(),
            ),
        ),
        errorCards: document.querySelectorAll(
  ...
  const toggle = currentMcp26825RecoveryPage.getByRole("button", {
    name: "Toggle summary",
  });
  await toggle.click();
  const closed = await readState();
  await currentMcp26825RecoveryPage.mouse.click(600, 600);
  const outside = await readState();
  await toggle.click();
  const repinned = await readState();
  await currentMcp26825RecoveryApp.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0]?.setContentSize(720, 680);
  });
  await currentMcp26825RecoveryPage.waitForFunction(
    () => window.innerWidth === 720 && window.innerHeight === 680,
    undefined,
    { timeout: 5_000 },
  );
  ...
  if (
    JSON.stringify(pinned.callGroups) !==
      JSON.stringify([
        ["Search OpenAI docs", "Fetch OpenAI doc"],
        ["Fetch OpenAI doc", "Search OpenAI docs", "Fetch OpenAI doc"],
      ]) ||
    JSON.stringify(pinned.answers) !==
      JSON.stringify([
        "CURRENT MCP 26.825 SUCCESS — Model Context Protocol — https://learn.chatgpt.com/docs/extend/mcp",
        "CURRENT MCP 26.825 RECOVERY — Model Context Protocol — https://learn.chatgpt.com/docs/extend/mcp",
      ]) ||
    pinned.errorCards !== 0 ||
    pinned.rowDisclosures !== 0 ||
    pinned.open !== "true" ||
    pinned.pinned !== "true" ||
    Math.abs((pinned.panel?.left ?? 0) - 864) > 1 ||
    Math.abs((pinned.panel?.height ?? 0) - 313) > 1 ||
    closed.open !== "false" ||
    closed.pinned !== "false" ||
    JSON.stringify(outside) !== JSON.stringify(closed) ||
    repinned.open !== "true" ||
    repinned.pinned !== "true" ||
    Math.abs((repinned.panel?.left ?? 0) - 864) > 1 ||
    compact.horizontalOverflow > 1
  ) {
    throw new Error(
      `26.825 Electron MCP lifecycle drifted: ${JSON.stringify({ closed, compact, outside, pinned, repinned })}`,
    );
  }
} finally {
  await currentMcp26825RecoveryApp.close();
}
...
for (const settingsLightScene of [
  ...
  },
  {
    ...
    heading: "General",
    id: "electron-general-settings-light",
    root: ".codex-ui-general-settings",
    theme: "light",
  },
  {
    frame: "workspace-general-settings",
    heading: "General",
    id: "electron-general-settings-light-compact",
    root: ".codex-ui-general-settings",
    theme: "light",
    windowSize: { height: 680, width: 720 },
  },
  {
    ...
    heading: "Appearance",
    id: "electron-appearance-settings-light",
    root: ".codex-ui-appearance-settings",
    theme: "light",
  },
  {
    frame: "workspace-appearance-settings",
    heading: "Appearance",
    id: "electron-appearance-settings-light-compact",
    root: ".codex-ui-appearance-settings",
    theme: "light",
    windowSize: { height: 680, width: 720 },
  },
