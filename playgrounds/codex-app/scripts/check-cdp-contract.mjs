...
const currentSidebarSpinnerPathData = [
  ...
  try {
    ...
    if (scene.view === "shell") {
      ...
      const notificationQueueScene =
        ...
        scene.id === "shell-notification-queue-light" ||
        scene.id === "shell-notification-queue-light-compact";
      const notificationSuccessStackScene =
        scene.id === "shell-notification-success-stack" ||
        scene.id === "shell-notification-success-stack-light" ||
        scene.id === "shell-notification-success-stack-light-compact";
      if (
        ...
        ((scene.id === "shell-restored" ||
          notificationQueueScene ||
          notificationSuccessStackScene) !== Boolean(contract.notification))
      ) {
        throw new Error(
          `${scene.id}: route lifecycle state failed: ${JSON.stringify(contract)}`,
        );
      }
      if (notificationQueueScene) {
        const lightNotification = scene.id.includes("-light");
        const successPaths = [
          "M12.1599 7.63617C12.3713 7.33596 12.7863 7.26372 13.0866 7.47504C13.3867 7.68642 13.4589 8.10153 13.2477 8.40179L9.28876 14.0268C9.17264 14.1917 8.98808 14.2954 8.7868 14.308C8.61044 14.319 8.43764 14.2592 8.30634 14.144L8.25262 14.0912L6.16962 11.7993L6.08954 11.6918C5.93136 11.4259 5.97666 11.0761 6.21454 10.8598C6.45225 10.6439 6.80379 10.6326 7.05341 10.8149L7.15399 10.9047L8.67841 12.5815L12.1599 7.63617Z",
          "M9.99506 2.81226C14.3664 2.81226 17.9101 6.35596 17.9101 10.7273C17.9101 15.0986 14.3664 18.6423 9.99506 18.6423C5.62372 18.6423 2.08002 15.0986 2.08002 10.7273C2.08002 6.35596 5.62372 2.81226 9.99506 2.81226ZM9.99506 4.14233C6.35826 4.14233 3.4101 7.0905 3.4101 10.7273C3.4101 14.3641 6.35826 17.3123 9.99506 17.3123C13.6319 17.3123 16.58 14.3641 16.58 10.7273C16.58 7.0905 13.6319 4.14233 9.99506 4.14233Z",
        ];
        ...
        const expectedSuccessBackground = lightNotification
          ? "rgb(237, 250, 242)"
          : "rgb(1, 28, 11)";
        const expectedSuccessColor = lightNotification
          ? "rgb(0, 162, 64)"
          : "rgb(64, 201, 119)";
        const expectedWarningBackground = lightNotification
          ? "rgb(255, 245, 240)"
          : "rgb(40, 17, 5)";
        const expectedWarningColor = lightNotification
          ? "rgb(226, 85, 7)"
          : "rgb(251, 106, 34)";
        if (
          ...
          notificationTones[0].backgroundColor !== expectedSuccessBackground ||
          notificationTones[0].color !== expectedSuccessColor ||
          notificationTones[1].backgroundColor !== expectedWarningBackground ||
          notificationTones[1].color !== expectedWarningColor
        ) {
          throw new Error(
        ...
        if (
          ...
          contract.notification.alert.style.backgroundColor !==
            expectedSuccessBackground ||
          ...
          contract.notification.alert.style.color !== expectedSuccessColor ||
      ...
      if (notificationSuccessStackScene) {
            ...
            worktreeFixtures: Array.from(
              ...
              (item) => {
                ...
                return {
                  ...
                  branchPathData: Array.from(
                    branch?.querySelectorAll("path") ?? [],
                    (path) => path.getAttribute("d"),
                  ),
                  branchRightInset:
                    rowBounds && branchBounds
                      ...
                      : null,
                  branchViewBox:
                    branch?.querySelector("svg")?.getAttribute("viewBox") ??
                    null,
                  ...
                  errorRect: error ? rect(error) : null,
                  errorViewBox: error?.getAttribute("viewBox") ?? null,
                  fixture: item.getAttribute(
                    "data-sidebar-worktree-status-fixture",
                  ),
                  ...
                  spinnerRect: spinner ? rect(spinner) : null,
                  spinnerViewBox:
                    spinner?.querySelector("svg")?.getAttribute("viewBox") ??
                    null,
                  secondaryStatus:
                    secondaryStatus?.getAttribute("data-status") ?? null,
                  secondaryVisualStatus:
    ...
    if (scene.scenario === "approval-current-26-825-file") {
      const optionsScene =
        ...
        return {
          approval: approval
            ? {
                ...
                backgroundColor: getComputedStyle(approval).backgroundColor,
                borderRadius: getComputedStyle(approval).borderRadius,
                identity:
                  approval
                ...
                identityIcon:
                  approval
                    ...
                    ?.getAttribute("data-current-build-icon") ?? null,
                preview: (() => {
                  const preview = approval.querySelector(
                    ".codex-ui-approval-file-preview",
                  );
                  return preview
                    ? {
                        additions:
                          preview.querySelector('[data-tone="added"]')
                            ?.textContent ?? null,
                        deletions:
                          preview.querySelector('[data-tone="removed"]')
                            ?.textContent ?? null,
                        directory:
                          preview
                            .querySelector(
                              ".codex-ui-approval-file-preview__directory",
                            )
                            ?.textContent?.trim() ?? null,
                        fileName:
                          preview
                            .querySelector(
                              ".codex-ui-approval-file-preview__name",
                            )
                            ?.textContent?.trim() ?? null,
                        rect: rect(preview),
                      }
      ...
      const pending = scene.frame?.endsWith("pending");
      const expectedFileName = scene.frame?.includes("allow-pending")
        ? "codex-ui-kit-26-825-approval-probe-allowed.txt"
        : "codex-ui-kit-26-825-approval-probe-pending.txt";
      if (
        pending &&
        (!currentFileApproval.approval ||
          Math.abs(
            currentFileApproval.approval.rect.left - (compact ? 16 : 222),
          ) > 1 ||
          Math.abs(
            currentFileApproval.approval.rect.top - (compact ? 487 : 627),
          ) > 1 ||
          Math.abs(
            currentFileApproval.approval.rect.width - (compact ? 688 : 736),
          ) > 1 ||
          Math.abs(currentFileApproval.approval.rect.height - 177) > 1 ||
          currentFileApproval.approval.backgroundColor !==
            "rgba(45, 45, 45, 0.96)" ||
          currentFileApproval.approval.borderRadius !== "25px" ||
          currentFileApproval.approval.identity !== "Edit files" ||
          currentFileApproval.approval.identityIcon !==
            "sidebar-project-menu-edit" ||
          currentFileApproval.approval.question !==
            "Allow ChatGPT to edit the following file?" ||
          currentFileApproval.approval.preview?.directory !==
            "/Users/demo-user/Desktop/" ||
          currentFileApproval.approval.preview?.fileName !== expectedFileName ||
          currentFileApproval.approval.preview?.additions !== "+1" ||
          currentFileApproval.approval.preview?.deletions !== "-0" ||
          Math.abs(
            currentFileApproval.approval.preview.rect.left -
              (compact ? 32 : 238),
          ) > 1 ||
          Math.abs(
            currentFileApproval.approval.preview.rect.top -
              (compact ? 571 : 711),
          ) > 1 ||
          Math.abs(
            currentFileApproval.approval.preview.rect.width -
              (compact ? 656 : 704),
          ) > 1 ||
          Math.abs(currentFileApproval.approval.preview.rect.height - 33) >
            1 ||
          !currentFileApproval.approval.actions.some(
            ({ shortcut, text }) => shortcut === "Esc" && text === "DenyEsc",
          ) ||
        ...
        throw new Error(
          `${scene.id}: current 26.825 file approval card contract failed: ${JSON.stringify(currentFileApproval)}`,
        );
      }
      if (
        optionsScene &&
        (!currentFileApproval.menu ||
          ...
          Math.abs(currentFileApproval.menu.rect.left - 519) > 1 ||
          Math.abs(currentFileApproval.menu.rect.top - 551) > 1 ||
          Math.abs(currentFileApproval.menu.rect.width - 168) > 1 ||
          Math.abs(currentFileApproval.menu.rect.height - 67.125) > 1)
      ) {
        throw new Error(
          `${scene.id}: current 26.825 file approval options contract failed: ${JSON.stringify(currentFileApproval)}`,
        );
      }
      if (optionsScene) {
        await page.keyboard.press("Escape");
        ...
        if (
          ...
          throw new Error(
            `${scene.id}: current 26.825 approval options did not dismiss cleanly: ${JSON.stringify(dismissed)}`,
          );
        }
      }
      if (
        ...
        (currentFileApproval.approval !== null ||
          ...
          currentFileApproval.permissionLabel !== "Ask for approval" ||
          currentFileApproval.permissionIcon !== "composer-permission-ask" ||
          !currentFileApproval.assistantText?.includes(
            scene.frame?.endsWith("denied")
              ? "Jamin，权限审批被拒绝，文件未创建。"
              : "已获批准，并使用 apply_patch 创建指定文件",
          ) ||
          (scene.frame?.endsWith("allowed") &&
            currentFileApproval.fileStatus !== "applied"))
      ) {
        throw new Error(
          `${scene.id}: current 26.825 file approval settlement contract failed: ${JSON.stringify(currentFileApproval)}`,
        );
      }
      if (scene.id === "approval-current-26-825-file-deny-pending") {
        await page
          .getByTestId("current-approval-request")
          .getByRole("button", { exact: true, name: "Deny" })
          .click();
        await page.waitForSelector(
          '.demo-root[data-frame="approval-current-26-825-file-denied"]',
        );
        await page
          .getByText("Jamin，权限审批被拒绝，文件未创建。", {
            exact: true,
          })
          .waitFor();
      }
      if (
        ...
        await page
          ...
          .click();
        await page.waitForSelector(
          '.demo-root[data-frame="approval-current-26-825-file-allowed"]',
        );
        await page
          .getByText("CURRENT 26.825 FILE APPROVAL ALLOWED", { exact: true })
          .first()
          .waitFor();
      }
    }
    ...
    if (currentSidebarWorktreeLifecycleScene) {
      ...
      const sharedInvalid = [active, failed, recovered, restored].some(
        (fixture) =>
          ...
          JSON.stringify(fixture.branchPathData) !==
            JSON.stringify(currentSidebarWorktreePathData) ||
          JSON.stringify(fixture.actionLabels) !== expectedActions ||
          fixture.actionsOpacity !== "0" ||
          !fixture.hasActions,
      );
      const invalid =
        sharedInvalid ||
        ...
        active?.statusRect?.height !== 20 ||
        active?.statusRect?.width !== 20 ||
        active?.spinnerRect?.height !== 16 ||
        active?.spinnerRect?.width !== 16 ||
        active?.spinnerViewBox !== "0 0 24 24" ||
        JSON.stringify(active?.spinnerPathData) !==
          JSON.stringify(currentSidebarSpinnerPathData) ||
        ...
        contract.sidebar.statusCounts.active !== 0 ||
        contract.sidebar.statusCounts.error !== 1 ||
        contract.sidebar.statusCounts.loading !== 1 ||
        contract.sidebar.statusCounts.unread !== 1 ||
        contract.sidebar.statusCounts.queued !== 0;
      if (invalid) {
        throw new Error(
          `${scene.id}: current worktree lifecycle contract failed: ${JSON.stringify(contract.sidebar)}`,
        );
      }
      await page
        .locator(
      ...
      const hovered = await page.evaluate(() => {
        ...
        return {
          actions: opacity(".codex-ui-app-sidebar__item-actions"),
          branch: opacity(
            ".codex-ui-app-sidebar__item-worktree-indicator",
          ),
          status: opacity(".codex-ui-app-sidebar__item-status"),
        };
      });
      if (
        hovered.actions !== "1" ||
        ...
        throw new Error(
          `${scene.id}: current worktree hover replacement failed: ${JSON.stringify(hovered)}`,
        );
      }
      await page.mouse.move(640, 100);
    }
