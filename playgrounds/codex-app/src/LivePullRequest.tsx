...
export function LivePullRequest({ projectToken, onWorkspaceChanged }: { projectToken?: string; onWorkspaceChanged?: () => void }) {
  ...
  const [busy, setBusy] = useState<"read" | "create" | "detail" | "diff" | "edit" | "merge" | "merge-status" | "cleanup" | null>(null);
  ...
  const close = () => { if (busy === "create" || busy === "edit" || busy === "merge" || busy === "cleanup") return; invalidate(); setOpen(false); };
  ...
  return <>
    ...
    <Dialog title="Prepare pull request" open={open} size="wide" returnFocusRef={trigger} onOpenChange={value => { if (!value) close(); }}
      footer={<><Button disabled={!!busy || !remote.trim()} onClick={() => void read()}>Refresh PRs</Button><Button disabled={!!busy || !preview || !!preview.pullRequests.length || !title.trim() || !base.trim() || base.trim() === preview.branch} onClick={() => void create()}>Confirm create PR</Button><Button disabled={busy === "create" || busy === "edit" || busy === "merge" || busy === "cleanup"} onClick={close}>Close</Button></>}>
      <div style={{ display: "grid", gap: 12, minWidth: 0, overflowWrap: "anywhere", maxHeight: "60vh", overflowY: "auto" }}>
        ...
        {busy && <p role="status">{busy === "cleanup" ? "Synchronizing main and cleaning the merged branch…" : busy === "merge" ? "Merging pull request…" : busy === "merge-status" ? "Checking merge result…" : busy === "edit" ? "Updating pull request…" : busy === "create" ? "Creating pull request…" : busy === "detail" ? "Reading PR details…" : busy === "diff" ? "Reading PR diff…" : "Reading pull requests…"}</p>}
        ...
        {open && <LivePullRequestMerge key={`${projectToken}:${remote}`} projectToken={projectToken} remote={remote.trim()} repository={preview?.repository} detail={editDraft ? null : detail} busy={!!busy} onBusy={setBusy} onWorkspaceChanged={onWorkspaceChanged} onMerged={() => { setDetail(null); setPreview(null); setPatch(null); setEditDraft(null); setResult(null); }} />}
