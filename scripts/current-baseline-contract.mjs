...
// Keep the newest installed build isolated until its affected shell/sidebar
// families are re-observed and the candidate record has been reviewed.
export const currentNewestCandidateBaselineFingerprint = Object.freeze({
  appAsarBytes: 342_674_749,
  appAsarSha256:
...
export function assertCurrentBaselineRecord(
  ...
  if (invalidRouteStacks.length > 0) {
    const observedRouteCounts = Object.fromEntries(
      invalidRouteStacks.map((state) => [
        state,
        Object.fromEntries(
          primaryRoutes.map((route) => [
            route,
            record.states[state].routes?.[route]?.length ?? 0,
          ]),
        ),
      ]),
    );
    throw new Error(
      `Current baseline record does not prove the primary navigation route stack: ${JSON.stringify({ invalidRouteStacks, observedRouteCounts })}`,
    );
  }
